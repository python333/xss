...
class Board(BaseModel):
    ...
    referer = TextField(index=True)

    class Meta:
        db_table = "words"

    @classmethod
    def new(cls, username, words, IP, referer):
        the_time = time.time()
        return cls.create(username=username, words=words, history_time=the_time, IP=IP, referer=referer)

    @classmethod
    def get_all(cls, username):
        words_query = cls.select().where(cls.username == username)
        return words_query

    @classmethod
    def dele_by_id(cls, word_id):
        x = cls.get_one(cls.id == word_id)
        if x:
            x.delete_instance()
